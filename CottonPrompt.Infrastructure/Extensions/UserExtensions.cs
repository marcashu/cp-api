...
namespace CottonPrompt.Infrastructure.Extensions
{
    internal static class UserExtensions
    {
        ...
        {
            var result = new GetUsersModel(Guid.Parse(graphUser.Id ?? string.Empty), graphUser.DisplayName ?? string.Empty, graphUser.UserPrincipalName ?? string.Empty, Enumerable.Empty<string>(), "", Enumerable.Empty<string>());
            return result;
        }
        ...
        {
            var result = new GetUsersModel(entity.Id, entity.Name, entity.Email, entity.UserRoles.Select(ur => ur.Role), entity.PaymentLink, entity.UserGroupUsers.Select(ur => ur.UserGroupId.ToString()));
            return result;
        }
